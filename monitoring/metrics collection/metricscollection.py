            ...
            if 'name_space' in entry:
                ...
                if 'host_location' and 'host_environment' in entry:
                    name_space = name_space.replace(
                        'avi.', 'avi.'+entry['host_location']+'.'+entry['host_environment']+'.')
                print('name=Custom Metrics|%s,value=%d,aggregator=OBSERVATION,time-rollup=CURRENT,cluster-rollup=INDIVIDUAL' %
                      (name_space, int(entry['metric_value'])))
    except:
...
def send_value_appdynamics_http(endpoint_info, appd_payload):
    try:
        ...
        if len(payload) > 0:
            ...
            resp = requests.post('http://%s:%s/api/v1/metrics' %
                                 (endpoint_info['server'], endpoint_info['server_port']), headers=headers, data=json.dumps(payload), timeout=15)
...
def send_value_datadog(endpoint_info, datadog_payload):
    try:
        ...
        headers = ({'content-type': 'application/json'})
        resp = requests.post('https://%s%s' % (endpoint_info['api_url'], endpoint_info['api_key']),
                             verify=False, headers=headers, data=json.dumps(payload), timeout=15)
        if resp.status_code != 202:
            print(resp)
    except:
        exception_text = traceback.format_exc()
    ...
    try:
        ...
        for entry in payload:
            ...
            if str(endpoint_info['auth-enabled']).lower() == 'true':
                resp = requests.post('%s://%s:%s/%s/_doc' % (endpoint_info['protocol'], endpoint_info['server'], endpoint_info['server_port'],
                                                             endpoint_info['index']), headers=headers, data=json.dumps(entry), timeout=15, auth=(endpoint_info['username'], endpoint_info['password']))
            else:
                resp = requests.post('%s://%s:%s/%s/_doc' % (endpoint_info['protocol'], endpoint_info['server'],
                                                             endpoint_info['server_port'], endpoint_info['index']), headers=headers, data=json.dumps(entry), timeout=15)
...
def send_value_graphite(endpoint_info, graphite_payload):
    try:
        ...
        for entry in graphite_payload:
            if 'name_space' in entry:
                name_space = (name_space_prefix+entry['name_space']).replace('.',
                                                                             '_').replace('||', '.').replace(' ', '_')
                # ----- used for migration from old script
                if 'host_location' and 'host_environment' in entry:
                    name_space = name_space.replace(
        ...
        for entry in influx_payload:
            ...
            if sys.getsizeof(message_list) > 4915:
                ...
                if auth_enabled == True:
                    resp = requests.post('%s://%s:%s/write?db=%s' % (endpoint_info['protocol'], endpoint_info['server'], endpoint_info['server_port'],
                                                                     endpoint_info['db']), verify=False, headers=headers, data=message, timeout=15, auth=(endpoint_info['username'], endpoint_info['password']))
                    message_list = []
                else:
                    resp = requests.post('%s://%s:%s/write?db=%s' % (endpoint_info['protocol'], endpoint_info['server'],
                                                                     endpoint_info['server_port'], endpoint_info['db']), verify=False, headers=headers, data=message, timeout=15)
                    message_list = []
...
def send_value_splunk_hec_metric(endpoint_info, splunk_payload):
    try:
        ...
        for entry in splunk_payload:
            temp_entry = entry
            keys_to_remove = ["location", "environment", "avicontroller",
                              "timestamp", "metric_value", "metric_name", "name_space"]
            ...
            resp = requests.post('%s://%s:%s/services/collector/event' % (endpoint_info['hec_protocol'], endpoint_info['server'], str(
                endpoint_info['hec_port'])), verify=False, headers=headers, data=json.dumps(payload), timeout=15)
...
def send_value_wavefront(endpoint_info, payload):
    try:
        ...
        if wf_proxy == False:
            ...
            resp = requests.post('https://'+wf_instance+'/report', verify=False,
                                 headers=headers, data=message, timeout=15)
            if resp.status_code != 202:
...
class avi_metrics():
    ...
    def avi_login(self):
        try:
            ...
            resp = requests.get('https://%s/api/cluster' % self.avi_cluster_ip,
                                verify=False, headers=headers, cookies=cookies, timeout=5)
            if resp.status_code == 200:
                return login
            else:
                login = requests.post('https://%s/login' % self.avi_cluster_ip, verify=False,
                                      data={'username': self.avi_user, 'password': self.avi_pass}, timeout=15)
    ...
    def avi_post(self, api_url, tenant, payload, api_version='17.2.1'):
        ...
        if 'avi-sessionid' in self.login.cookies.keys():
            ...
            cookies['sessionid'] = self.login.cookies['sessionid']
        headers = ({"X-Avi-Tenant": "%s" % tenant, 'content-type': 'application/json', 'referer': 'https://%s' % self.avi_controller,
                    'X-CSRFToken': dict(self.login.cookies)['csrftoken'], 'X-Avi-Version': '%s' % api_version})
    ...
    def controller_to_poll(self):
        ...
        if 'avi-sessionid' in self.login.cookies.keys():
            ...
            cookies['sessionid'] = self.login.cookies['sessionid']
        headers = ({"X-Avi-Tenant": "admin", 'content-type': 'application/json'})
        resp = (requests.get('https://%s/api/%s' % (self.avi_cluster_ip, 'cluster/runtime'),
                             verify=False, headers=headers, cookies=cookies, timeout=50)).json()
        follower_list = []
        ...
        try:
            ...
            for t in self.tenants:
                if self.vs_metrics == True or self.vs_runtime == True:
                    vs_inv = self.avi_request(
                        'virtualservice?fields=cloud_ref,tenant_ref,se_group_ref&page_size=200&include_name=true&'+vs_runtime, t['name'])
                    if vs_inv.status_code == 403:
                        ...
                        while 'next' in resp:
                            page_number += 1
                            resp = self.avi_request(
                                'virtualservice?fields=cloud_ref,tenant_ref,se_group_ref&page_size=200&include_name=true&page='+str(page_number)+vs_runtime, t['name']).json()
                ...
                if self.se_metrics == True or self.se_runtime == True:
                    se_inv = self.avi_request(
                        'serviceengine?fields=cloud_ref,tenant_ref,se_group_ref,vs_refs&page_size=200&include_name=true'+se_runtime, t['name'])
                    if se_inv.status_code == 403:
                        ...
                        while 'next' in resp:
                            page_number += 1
                            resp = self.avi_request(
                                'serviceengine?fields=cloud_ref,tenant_ref,se_group_ref,vs_refs&page_size=200&include_name=true&page='+str(page_number)+se_runtime, t['name']).json()
                        ...
                        if se_inv['count'] > 0:
                            for s in se_inv['results']:
                                ...
                                se_dict[s['uuid']]['se_group_uuid'] = s['se_group_ref'].split(
                                    '/serviceenginegroup/')[1].rsplit('#')[0]
                ...
                if self.pool_metrics == True or self.pool_runtime == True:
                    ...
                    if pool_inv.status_code == 403:
                        ...
                        while 'next' in resp:
                            page_number += 1
                            resp = self.avi_request(
                                'pool-inventory?include_name=true&page_size=200&page='+str(page_number), t['name']).json()
                ...
                # ---------------
                seg_inv = self.avi_request(
                    'serviceenginegroup?fields=max_vs_per_se,cloud_ref,tenant_ref&include_name&page_size=200', t['name'])
            ...
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func gen_inventory_dict completed, executed in '+temp_total_time+' seconds')
    ...
    def srvc_engn_vs_count(self):
        try:
            ...
            for s in self.se_dict:
                if len(self.se_dict[s]['virtualservices']) > 0:
                    ...
                    temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                        '||serviceengine||%s||vs_count' % self.se_dict[s]['name']
            ...
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func srvc_engn_vs_count completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def srvc_engn_count(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func srvc_engn_count completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def srvc_engn_stats(self, tenant):
        try:
            ...
            if self.se_realtime == True:
                ...
                realtime_stats = self.avi_post(
                    'analytics/metrics/collection?pad_missing_data=false', tenant, payload).json()
                    ...
                    if tenant == 'admin' or (tenant != 'admin' and _tenant != 'admin'):
                        for entry in se_stat['series']['collItemRequest:AllSEs'][s]:
                            if 'data' in entry:
                                ...
                                temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                    '||serviceengine||%s||%s' % (se_name, entry['header']['name'])
                                if self.se_realtime == True:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func srvc_engn_stats completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def virtual_service_stats(self, tenant):
        try:
            ...
            # -----
            endpoint_payload_list = []
            payload = {'metric_requests': [{'step': 300, 'limit': 1, 'id': 'allvs',
                                            'entity_uuid': '*', 'metric_id': self.vs_metric_list}]}
            ...
            if self.vs_realtime == True:
                payload = {'metric_requests': [{'step': 5, 'limit': 1, 'id': 'allvs',
                                                'entity_uuid': '*', 'metric_id': self.vs_metric_list}]}
                realtime_stats = self.avi_post(
                    'analytics/metrics/collection?pad_missing_data=false', tenant, payload).json()
            # -----
                    ...
                    if tenant == 'admin' or (tenant != 'admin' and _tenant != 'admin'):
                        for m in vs_stats['series']['allvs'][v]:
                            ...
                            if 'data' in m:
                                ...
                                temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                    '||virtualservice||%s||%s' % (vs_name, metric_name)
                                if self.vs_realtime == True:
                                    if 'series' in realtime_stats:
            ...
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip+': func virtual_service_stats completed for tenant: ' +
                  tenant+', executed in '+temp_total_time+' seconds')
        except:
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func virtual_service_stats encountered an error for tenant '+tenant)
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
    ...
    def vs_metrics_per_se_threaded(self):
        try:
            ...
            if (float(major) >= 17.2 and float(minor) >= 8) or float(major) >= 18.1:
                ...
                temp_total_time = str(time.time()-temp_start_time)
                print(str(datetime.now())+' '+self.avi_cluster_ip +
                      ': func vs_metrics_per_se_threaded completed, executed in '+temp_total_time+' seconds')
        except:
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
    ...
    def vs_metrics_per_se(self, tenant):
        try:
            temp_start_time = time.time()
            endpoint_payload_list = []
            payload = {'metric_requests': [{'step': 300, 'limit': 1, 'id': 'vs_metrics_by_se', 'entity_uuid': '*',
                                            'serviceengine_uuid': '*', 'include_refs': True, 'metric_id': self.vs_metric_list}]}
            vs_stats = self.avi_post(
                'analytics/metrics/collection?include_name=true&pad_missing_data=false', tenant, payload).json()
            # ----- this will pull 5 sec stats for vs that have realtime stat enabled
            if self.vs_realtime == True:
                payload = {'metric_requests': [{'step': 5, 'limit': 1, 'id': 'vs_metrics_by_se', 'entity_uuid': '*',
                                                'serviceengine_uuid': '*', 'include_refs': True, 'metric_id': self.vs_metric_list}]}
                realtime_stats = self.avi_post(
                    'analytics/metrics/collection?include_name=true&pad_missing_data=false', tenant, payload).json()
            # ------
                            ...
                            for d in vs_stats['series']['vs_metrics_by_se'][entry]:
                                if 'data' in d:
                                    if 'serviceengine_ref' in d['header']:
                                        ...
                                        temp_payload['name_space'] = 'avi||'+self.avi_cluster_name+'||serviceengine||%s||virtualservice_stats||%s||%s' % (
                                            se_name, vs_name, temp_payload['metric_name'])
                                        endpoint_payload_list.append(temp_payload)
                if len(endpoint_payload_list) > 0:
                    send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
                temp_total_time = str(time.time()-temp_start_time)
                print(str(datetime.now())+' '+self.avi_cluster_ip+': func vs_metrics_per_se completed tenant: ' +
                      tenant+', executed in '+temp_total_time+' seconds')
        except:
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func vs_metrics_per_se for tenant: '+tenant+', encountered an error')
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
    ...
    def vs_oper_status(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func vs_oper_status completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def active_pool_members(self):
        try:
                ...
                try:
                    ...
                    if 'num_servers' in p['runtime']:
                        ...
                        for vs_entry in vs_list:
                            ...
                            temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                '||virtualservice||%s||pool||%s||%s' % (vs_entry, pool_name, 'pool_members_enabled')
                            endpoint_payload_list.append(temp_payload)
                            # ----- pool members up
                            ...
                            temp1_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                '||virtualservice||%s||pool||%s||%s' % (vs_entry, pool_name, 'pool_members_up')
                            ...
                            temp2_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                '||virtualservice||%s||pool||%s||%s' % (vs_entry, pool_name, 'pool_members')
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func active_pool_members completed, executed in '+temp_total_time+' seconds')
        except:
            ...
            for s in self.se_dict:
                if s not in discovered_se:
                    ...
                    if 'hb_status' in self.se_dict[s]['results']['runtime']:
                        ...
                        temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                            '||serviceengine||%s||%s' % (self.se_dict[s]['name'], 'missed_heartbeats')
                        endpoint_payload_list.append(temp_payload)
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func se_missed_hb completed, executed in '+temp_total_time+' seconds')
        except:
            ...
            for s in self.se_dict:
                if s not in discovered_se:
                    ...
                    if 'se_connected' in self.se_dict[s]['results']['runtime']:
                        ...
                        temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                            '||serviceengine||%s||%s' % (self.se_dict[s]['name'], 'connected_state')
                        endpoint_payload_list.append(temp_payload)
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func se_connected completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def cluster_status(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func cluster_status completed, executed in '+temp_total_time+' seconds')
        except:
                    ...
                    for s in subnets:
                        if 'subnet_runtime' in s['runtime'].keys():
                            ...
                            if pool_size > 0:
                                ...
                                temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                    '||networks||%s||used' % network_name
                                endpoint_payload_list.append(temp_payload)
                if len(endpoint_payload_list) > 0:
                    send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
                temp_total_time = str(time.time()-temp_start_time)
                print(str(datetime.now())+' '+self.avi_cluster_ip +
                      ': func avi_subnet_usage completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def virtual_service_hosted_se(self):
        try:
                ...
                if 'service_engine' in self.vs_dict[v]['results']['runtime']['vip_summary'][0]:
                    ...
                    for r in self.vs_dict[v]['results']['runtime']['vip_summary']:
                        for s in r['service_engine']:
                            ...
                            temp_payload['metric_value'] = 1
                            temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                '||virtualservice||%s||serviceengine||%s' % (vs_name, se_name)
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func virtual_service_hosted_se completed, executed in '+temp_total_time+' seconds')
        except:
                        ...
                        if 'service_engine' in a:
                            for s in a['service_engine']:
                                if s['primary'] == True:
                                    ...
                                    temp_payload['metric_value'] = 1
                                    temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                                        '||virtualservice||%s||primary_se||%s' % (vs_name, se_name)
                                    endpoint_payload_list.append(temp_payload)
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func virtual_service_primary_se completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def license_usage(self):
        try:
            ...
            if lic_cores != None:
                ...
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
                print(str(datetime.now())+' '+self.avi_cluster_ip +
                      ': func license_usage completed, executed in '+temp_total_time+' seconds')
        except:
        ...
        try:
            ...
            for s in self.se_dict:
                ...
                if se_name not in se_vs:
                    seg = self.se_dict[s]['results']['se_group_ref'].rsplit(
                        '#', 1)[0].split('/api/serviceenginegroup/')[1]
            ...
            for entry in se_vs:
                ...
                temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                    '||serviceengine||%s||vs_capacity_used' % entry
                endpoint_payload_list.append(temp_payload)
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func service_engine_vs_capacity completed, executed in '+temp_total_time+' seconds')
        except:
        ...
        try:
            if datetime.now().hour % 6 == 0:  # ----- run once every 6 hours
                ...
                for l in licenses['licenses']:
                    ...
                    temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                        '||licensing||expiration_days||'+license_id
                    endpoint_payload_list.append(temp_payload)
                    send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
                temp_total_time = str(time.time()-temp_start_time)
                print(str(datetime.now())+' '+self.avi_cluster_ip +
                      ': func license_expiration completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def get_serviceengine_version(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func get_serviceengine_version completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def get_controller_version(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func get_avi_version completed, executed in '+temp_total_time+' seconds')
        except:
    ...
    def pool_server_stats_threaded(self):
        try:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func pool_server_stats_threaded completed, executed in '+temp_total_time+' seconds')
        except:
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
    ...
    def pool_server_stats(self, tenant):
        try:
            ...
            if self.pool_realtime == True:
                ...
                realtime_stats = self.avi_post(
                    'analytics/metrics/collection?pad_missing_data=false&dimension_limit=1000&include_name=true&include_refs=true', tenant, payload).json()
            if 'series' in resp:
                        ...
                        if p not in discovered_servers:
                            ...
                            for d in resp['series']['collItemRequest:AllServers'][p]:
                                if 'data' in d:
                                    ...
                                    temp_payload['timestamp'] = int(time.time())
                                    temp_payload['pool_name'] = pool_name
                                    temp_payload['tenant'] = self.pool_dict[d['header']
                                                                            ['pool_ref'].rsplit('#', 1)[0].split('/api/pool/')[1]]['tenant']
                                    temp_payload['cloud'] = self.pool_dict[d['header']
                                                                           ['pool_ref'].rsplit('#', 1)[0].split('api/pool/')[1]]['cloud']
            ...
            if len(endpoint_payload_list) > 0:
                send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip+': func pool_server_stats for tenant ' +
                  tenant+', executed in '+temp_total_time+' seconds')
        except:
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func pool_server_stats  encountered an error for tenant '+tenant)
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
        ...
        try:
            ...
            if (float(major) >= 17.2 and float(minor) >= 5) or float(major) >= 18.1:  # ----- controller metrics api introduced in 17.2.5
                ...
                for c in cluster['nodes']:
                    ...
                    resp = self.avi_request('analytics/metrics/controller/%s/?metric_id=%s&limit=1&step=300&?aggregate_entity=False' % (
                        c['vm_uuid'], self.controller_metric_list), 'admin').json()
                ...
                for n in temp_list:
                    ...
                    for m in n['series']:
                        ...
                        temp_payload['metric_value'] = m['data'][0]['value']
                        temp_payload['name_space'] = 'avi||'+self.avi_cluster_name + \
                            '||controller||%s||%s' % (node, metric_name)
                        endpoint_payload_list.append(temp_payload)
                if len(endpoint_payload_list) > 0:
                    send_metriclist_to_endpoint(self.endpoint_list, endpoint_payload_list)
            else:
            ...
            temp_total_time = str(time.time()-temp_start_time)
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func controller_cluster_metrics, executed in '+temp_total_time+' seconds')
        except:
            print(str(datetime.now())+' '+self.avi_cluster_ip +
                  ': func controller_cluster_metrics encountered an error encountered an error')
            exception_text = traceback.format_exc()
            print(str(datetime.now())+' '+self.avi_cluster_ip+': '+exception_text)
    ...
    def gather_metrics(self):
        try:
            ...
            if self.login.status_code == 200:
                ...
                print(str(datetime.now())+' '+self.avi_cluster_ip +
                      ': controller specific tests have completed, executed in '+total_time+' seconds')
                endpoint_payload_list = []
                temp_payload = self.payload_template.copy()
                temp_payload['timestamp'] = int(time.time())
