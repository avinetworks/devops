...
type AviSession struct {
	...
	// headers to be passed to each Request
...
func SetHeaders(headers map[string]string) func(*AviSession) error {
	return func(sess *AviSession) error {
		return sess.setHeaders(headers)
	}
}

func (avisess *AviSession) setHeaders(headers map[string]string) error {
	avisess.headers = headers
	return nil
}
...
func (avisess *AviSession) newAviRequest(verb string, url string, payload io.Reader, tenant string) (*http.Request, AviError) {
	...
	for key, val := range avisess.headers {
